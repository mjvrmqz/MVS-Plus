            ...
            if (prop.propertyType === PropertyType.PROPERTY && prop.numKeys > 0) {
                for (var k = 1; k <= prop.numKeys; k++) {
                    if (prop.keySelected(k)) {
                        ...
                        layer.property("Marker").setValueAtTime(prop.keyTime(k), marker);
                    }
                }
            } else if (prop.numProperties > 0) for (var p = 1; p <= prop.numProperties; p++) processProperty(prop.property(p));
        }
