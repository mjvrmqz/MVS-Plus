{
    function setInOutForAllLayers() {
        ...
        for (var i = 1; i <= layers.length; i++) {
            var layer = layers[i];
            var props = [layer.property("Position"), layer.property("Rotation"), layer.property("Scale"), layer.property("Opacity")];
            var firstKF = null; var lastKF = null;
            for (var p = 0; p < props.length; p++) {
                var prop = props[p];
                if (prop && prop.numKeys > 0) {
                    var first = prop.keyTime(1); var last = prop.keyTime(prop.numKeys);
                    if (firstKF === null || first < firstKF) firstKF = first;
                    if (lastKF === null || last > lastKF) lastKF = last;
                }
            }
            if (firstKF !== null && lastKF !== null) {
                layer.inPoint = Math.max(0, firstKF - 1);
                layer.outPoint = Math.min(comp.duration, lastKF + 1);
            }
        }
        app.endUndoGroup();
    }
